...
async function diagnose() {
    ...
    for (const table of tables) {
        try {
            // Check global count
            const globalRes = await fetch(`${URL}/${table}?select=*`, {
                method: 'HEAD',
            ...
            // Try to get 1 record without filter to see columns
            const sampleRes = await fetch(`${URL}/${table}?select=*&limit=1`, {
            ...
            if (sample && sample[0]) {
                ...
                // If it has user_id or owner_id, check owner count
                const idCol = sample[0].user_id ? 'user_id' : (sample[0].owner_id ? 'owner_id' : (table === 'profiles' ? 'id' : null));
                if (idCol) {
                    const ownerRes = await fetch(`${URL}/${table}?select=*&${idCol}=eq.${OWNER_ID}`, {
                        method: 'HEAD',
                        headers: { 'apikey': KEY, 'Authorization': `Bearer ${KEY}`, 'Prefer': 'count=exact' }
                    });
                    const ownerCount = ownerRes.headers.get('content-range')?.split('/')[1] || 0;
                    console.log(`   Owner Count (${idCol}): ${ownerCount}`);
                }
            } else {
                console.log(`   (No records found or RLS blocked)`);
            }
        } catch (e) {
